...
namespace HarmonyLibTests.IL
{
	...
	public class DynamicArgumentPatches
	{
		...
		private static bool General(string typeName, int token, object instance, object[] args)
		{
		...
		private static MethodInfo m_General = SymbolExtensions.GetMethodInfo(() => General("", 0, null, new object[0]));
		private static MethodInfo m_Transpiler = SymbolExtensions.GetMethodInfo(() => Transpiler(null, null, null));

		private static IEnumerable<CodeInstruction> Transpiler(MethodBase original,
		...
		private static IEnumerable<CodeInstruction> CreateDefaultCodes(ILGenerator generator, Type type)
		{
		...
		private static MethodInfo[] methods = new MethodInfo[]
		{
		...
		public void SendingArguments()
		{
			...
			methods.Do(m => { harmony.Patch(m, transpiler: new HarmonyMethod(m_Transpiler)); });
